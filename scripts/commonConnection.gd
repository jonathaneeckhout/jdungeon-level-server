...
func save_character(character_name: String, level_name: String, pos: Vector2, gold: int):
	var new_req = save_character_request.new()
	add_child(new_req)
	var res = await new_req.save_character(character_name, level_name, pos, gold, cookie)
	new_req.queue_free()
	return res
